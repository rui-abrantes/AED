 ...
 *    (none)
 */
void deleteNextNodeLinkedList(LinkedList * curr, void (* freeItemFnt)(Item) )
{
    LinkedList *aux = curr->next;;

    /* if there is no next simply return */
    if (aux == (LinkedList*) NULL) return;

    /* save following pointer */
    aux = aux-> next;

    /* delete curr->next */
    freeItemFnt(curr->next->this);
    free(curr->next);
...
/*
 *  Function:
 *    deleteHeaderLinkedList
 *
 *  Description:
 *    Deletes the header of the LinkedList
 *
 *  Arguments:
 ...
 *    (none)
 */
LinkedList *deleteHeaderLinkedList(LinkedList * head, void (* freeItemFnt)(Item)) {
    LinkedList *aux = head->next;;

    /* delete curr->next */
    freeItemFnt(head->this);
    free(head);
